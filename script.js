(() => {
    ...
    if (screen.image) {
      return `
        <div class="app-screen screenshot-screen">
          <img src="${screen.image}" alt="Stylish shopping app home screen" />
        </div>
      `;
    }
  ...
  function getCards() {
    return [...carousel.querySelectorAll(".phone-card")];
  }
  ...
  function renderCarousel() {
    ...
    cards.forEach((card, index) => {
      ...
      card.style.opacity = distance > 1 ? "0.24" : "1";
      card.style.filter = distance ? "saturate(0.55)" : "none";
      card.style.transform = `translateX(calc(-50% + ${offset * 116}px)) translateZ(${-distance * 120}px) rotateY(${offset * -34}deg) scale(${scale})`;
    });
  ...
  stage.addEventListener(
    "touchstart",
    (event) => startDrag(event.touches[0].clientX),
    { passive: true },
  );
  stage.addEventListener(
    "touchend",
    (event) => endDrag(event.changedTouches[0].clientX),
    { passive: true },
  );
